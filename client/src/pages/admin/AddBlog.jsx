...
const AddBlog = () => {
    ...
    return (
        <form onSubmit={formSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
            <div className={'bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 shadow rounded'}>
                <p>Upload Thumbnail</p>
                <label htmlFor={'image'}>
                    <img src={!image ? assets.upload_area : URL.createObjectURL(image)} alt={'area icon'} className={'mt-2 h-16 rounded cursor-pointer'}/>
                    <input onChange={(e)=> setImage(e.target.files[0])} type="file" id={'image'} hidden required />
                </label>
                <p className={'mt-4'}>Blog Title</p>
                <input type="text" placeholder='Type here' required className='w-full max-w-1g mt-2 p-2 border border-gray-300 outline-none rounded'
